...
meta_donor <- read.csv("https://raw.githubusercontent.com/pedrohpro/RNAseq_Autism/main/bulkBrain_donorMetadata.csv", sep = ";", row.names = 1)

genes_asd <- read.csv("https://raw.githubusercontent.com/pedrohpro/RNAseq_Autism/main/genesofinterest.csv", sep = ";")

genes_down <- read.csv("https://raw.githubusercontent.com/pedrohpro/RNAseq_Autism/main/genes_down.csv", sep = ";")

patients_mutations <- read.csv("https://raw.githubusercontent.com/pedrohpro/RNAseq_Autism/main/pacientsmutations.csv", sep = ";", row.names = 1)

# Maybe I should remove this part below
# Down genes
down_complement <- paste(genes_down$ensembl_id, collapse = "|")

# Checking genes
enriched_genes <- genes_asd$genes_asdnterest
any(duplicated(enriched_genes))
genes_asd$pathway <- factor(genes_asd$pathway)
summary(genes_asd$pathway)
```
...
meta_donor <- mutate_all(meta_donor, as.character)

# Changing some names in the donor's metadata data frame for better visualizing
# 4M will become 0.3Y
meta_donor$Age <- gsub("4M", "0.3Y", meta_donor$Age)
# 6M will become 0.5Y
meta_donor$Age <- gsub("6M", "0.5Y", meta_donor$Age)
# 301days will become 0.8Y
meta_donor$Age <- gsub("301days", "0.8Y", meta_donor$Age)
# 12M will become 1Y
meta_donor$Age <- gsub("12M", "1Y", meta_donor$Age)
# 2Y178d will become 2.5Y
meta_donor$Age <- gsub("2Y178d", "2.5Y", meta_donor$Age)
# 2Y305d will become 2.8Y
meta_donor$Age <- gsub("2Y305d", "2.8Y", meta_donor$Age)
# 8Y90d will become 8.3Y
meta_donor$Age <- gsub("8Y90d", "8.3Y", meta_donor$Age)
# 10Y265d will become 10.7Y
meta_donor$Age <- gsub("10Y265d", "10.7Y", meta_donor$Age)

# Creating a donor column
meta_donor$Donor <- rownames(meta_donor)

# Chagning Sequencing.Site column name
names(meta_donor)[names(meta_donor) == "Sequencing.Site"] <- "SequencingSite"
```
...
meta <- data.frame(matrix("vazio", nrow = length(colnames(counts)), ncol = length(colnames(meta_donor))), stringsAsFactors=FALSE)

colnames(meta) <- colnames(meta_donor) # colocar nome nas colunas
...
for (i in 1:length(rownames(meta))){
  meta[i,] <- meta_donor[grepl(gsub("\\..*", "", colnames(counts)[i]), rownames(meta_donor)), ]
}
...
meta$GroupedRegion <- factor(meta$GroupedRegion, levels = c("Frontal_lobe", "Parietal_lobe", "Temporal_lobe", "Occipital_lobe", "Limbic_system", "Cerebellum"))
```
...
#  time_subs[[i]]$cts_raw <- list(counts[,filter])
#  time_subs[[i]]$meta <- list(meta[filter,])
#}

# Re-leveling all columns of all subsets: there isnt always samples for each level
# (NOTE: re-factors in alphabetical order! But age levels order was preserved)
#time_subs <- map()
...
for (i in levels(meta$GroupedAge)){
  filter <- grepl(i, meta$GroupedAge)
  time_metasubs <- append(time_metasubs, list(meta[filter,]))
  time_countsubs <- append(time_countsubs, list(counts[,filter]))
}

# Re-leveling all columns of all subsets: there isnt always samples for each level
# (NOTE: re-factors in alphabetical order! But age levels order was preserved)
time_metasubs <- map(time_metasubs, ~ mutate_all(.x, factor))
...
for (i in discard(levels(meta$GroupedRegion), ~ .x == "Ganglionic_eminence")){
  ...
  space_metasubs <- append(space_metasubs, list(meta[filter,]))
  space_countsubs <- append(space_countsubs, list(counts[,filter]))
}

# Re-leveling all factors so that no factor level has 0 samples (all col 4 all df)
space_metasubs <- map(space_metasubs, ~ mutate_all(.x, factor))
...
print(paste0("Time meta and counts subsets have the same (rownames and colnames) names? ", all(map2_lgl(time_countsubs, time_metasubs, ~ all(colnames(.x) %in% rownames(.y))))))

print(paste0("Time meta and counts subsets have the same (rownames and colnames) order? ", all(map2_lgl(time_countsubs, time_metasubs, ~ all(colnames(.x) == rownames(.y))))))

print(paste0("Space meta and counts subsets have the same (rownames and colnames) names? ", all(map2_lgl(space_countsubs, space_metasubs, ~ all(colnames(.x) %in% rownames(.y))))))

print(paste0("Space meta and counts subsets have the same (rownames and colnames) order? ", all(map2_lgl(space_countsubs, space_metasubs, ~ all(colnames(.x) == rownames(.y))))))
...
# DESCOBRIR OUTRO JEITO DE FAZER ISSO SEM AuxCol
...
#### Comparing batch effect and desired variable (region) {style="background-color: #449944; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px;"}

```{r}
# Choosen genes
View(counts[grepl("CACNA1H|RELN", rownames(counts)),])
