...
# Bulk RNA-seq - control *post mortem* brains {style="background-color: #4b6043; color: #FFF685;  padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### Useful commands {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
# Data wrangling {style="background-color: #0049b7; color: #FFF685;  padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

### Library importation {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### Data importation {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

```{r}

counts <- read.delim("https://raw.githubusercontent.com/pedrohpro/RNAseq_Autism/main/bulkBrain_counts.txt", row.names = 1)

metadoador <- read.csv("https://raw.githubusercontent.com/pedrohpro/RNAseq_Autism/main/bulkBrain_donorMetadata.csv", sep = ";", row.names = 1)

genesofi <- read.csv("https://raw.githubusercontent.com/pedrohpro/RNAseq_Autism/main/genesofinterest.csv", sep = ";")

# Checking genes
enriched_genes <- genesofi$genesofinterest
any(duplicated(enriched_genes))
genesofi$pathway <- factor(genesofi$pathway)
summary(genesofi$pathway)
```

### Removing samples {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

Here we remove outliers and problematic samples from the counts matrix.

All samples from the MSC region were removed because the actual region the acronym refers to is not in the metadata and we can not safely assume what region it refers to.
...
### Cleaning counts matrix {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

```{r}
# Excluding genes with low counts (will be done later in the code)
counts <- counts[rowSums(counts) >= 50,] # removes genes that have less than 50 counts in all samples

# Changing the region name M1CS1C to MS1 for better visualization
colnames(counts) <- gsub("M1CS1C", "MS1", colnames(counts))

# Changing "|" to "-" for Seurat
...
### Adjusting donor metadata data frame {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

```{r}
# Necessary step to create metadata dataframe
for (j in 1:ncol(metadoador)){
  metadoador[,j] <- as.character(metadoador[,j])
}

# Changing some names in the donor's metadata data frame for better visualizing
# 4M will become 0.3Y
metadoador["HSB121", "Age"] <- "0.3Y"
metadoador["HSB132", "Age"] <- "0.3Y"
metadoador["HSB139", "Age"] <- "0.3Y"
# 6M will become 0.5Y
metadoador["HSB131", "Age"] <- "0.5Y"
# 301days will become 0.8Y
metadoador["HSB171", "Age"] <- "0.8Y"
# 12M will become 1Y
metadoador["HSB122", "Age"] <- "1Y"
# 2Y178d will become 2.5Y
metadoador["HSB173", "Age"] <- "2.5Y"
# 2Y305d will become 2.8Y
metadoador["HSB172", "Age"] <- "2.8Y"
# 8Y90d will become 8.3Y
metadoador["HSB174", "Age"] <- "8.3Y"
# 10Y265d will become 10.7Y
metadoador["HSB175", "Age"] <- "10.7Y"

# Chagning Sequencing.Site column name
names(metadoador)[names(metadoador) == "Sequencing.Site"] <- "SequencingSite"
```

### Creating sample metadata data frame {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

```{r}


# Creating the empty metadata dataframe for all counts
meta <- data.frame(matrix("vazio", nrow = length(colnames(counts)), ncol = length(colnames(metadoador))), stringsAsFactors=FALSE)

colnames(meta) <- colnames(metadoador) # colocar nome nas colunas
...
for (i in 1:length(rownames(meta))){
  meta[i,] <- metadoador[grepl(gsub("\\..*", "", colnames(counts)[i]), rownames(metadoador)), ]
}
...
### Grouping periods {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
# Grouping developmental Periods
# 8 to 12 weeks
# 13 to 19 weeks
# 20 to 40 weeks
# 0 to 2 years
...
# the 280 days are the number of days of gestation. meta$Days is the number of days since fertilization and not birth
for (k in 1:length(rownames(meta))){
  if (meta$Days[k] <= 12*7){
    meta$GroupedAge[k] <- "8_12PCW"
  }else if (meta$Days[k] <= 19*7){
    meta$GroupedAge[k] <- "13_19PCW"
  }else if (meta$Days[k] <= 40*7){
    meta$GroupedAge[k] <- "20_40PCW"
  }else if (meta$Days[k] <= 280 + 365*2){
    meta$GroupedAge[k] <- "0_2YEARS"
  }else if (meta$Days[k] <= 280 + 365*10){
    meta$GroupedAge[k] <- "3_10YEARS"
  }else if (meta$Days[k] <= 280 + 365*22){
    meta$GroupedAge[k] <- "11_22YEARS"
  }else {
    meta$GroupedAge[k] <- "23_40YEARS"
  }
}
...
### Grouping regions {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

This step was done after analyzing the dimensionality reduction techniques. Each group was defined based on biological and statistical evidence. The paper will explain it in detail.

```{r}
# Grouping regions
for (h in 1:nrow(meta)){
  if (any(meta$Region[h] %in% c("A1C","ITC","TC","STC"))){
    meta$GroupedRegion[h] <- "Temporal_lobe"
  }else if (any(meta$Region[h] %in% c("IPC","S1C","PC"))){
    meta$GroupedRegion[h] <- "Parietal_lobe"
  }else if (any(meta$Region[h] %in% c("M1C","DFC","MFC","OFC","VFC","MS1"))){
    meta$GroupedRegion[h] <- "Frontal_lobe"
  }else if (any(meta$Region[h] %in% c("V1C","OC"))){
    meta$GroupedRegion[h] <- "Occipital_lobe"
  }else if (any(meta$Region[h] %in% c("AMY","HIP","STR","MD","DTH"))){
    meta$GroupedRegion[h] <- "Limbic_system"
  }else if (any(meta$Region[h] %in% c("CBC","URL"))){
    meta$GroupedRegion[h] <- "Cerebellum"
  }else {
    meta$GroupedRegion[h] <- "Ganglionic_eminence"
  }
}

# Perhaps it would be interesting to re-level them too (but not crucial for visualization)
meta$GroupedRegion <- factor(meta$GroupedRegion)
```

### Final touch-ups on metadata {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

```{r}
# Reorganizing the metadata columns
meta <- meta %>% relocate(Age, GroupedAge, Region, GroupedRegion, Hemisphere, Sex, Ethnicity, SequencingSite, pH)
...
meta <- subset(meta, select = -Days)

# Creating an auxiliary column for sample visualization 
meta$AuxCol <- 0
...
#### Creating time subsets {style="background-color: #449944; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
for (i in levels(meta$GroupedAge)){
  ...
  countsub <- counts[,filter]
  time_countsubs <- append(time_countsubs, list(countsub))
  metasub <- meta[filter,]
  # Re-factor so that there is only one level
  metasub$GroupedAge <- factor(metasub$GroupedAge)
  time_metasubs <- append(time_metasubs, list(metasub))
}


# Re-leveling all columns of all subsets: there isnt always samples for each level
# (NOTE: it will re-factor in alphabetical order!)
for (i in 1:length(time_metasubs)){
  for (j in colnames(time_metasubs[[i]])){
    time_metasubs[[i]][,j] <- factor(time_metasubs[[i]][,j])
  }
}

```

#### Creating space subsets {style="background-color: #449944; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
space_metasubs <- list()
space_countsubs <- list()

for (i in levels(meta$GroupedRegion)[!grepl("Ganglionic_eminence", levels(meta$GroupedRegion))]){
  space_metasubs <- append(space_metasubs, list(meta[grepl(i, meta$GroupedRegion),]))
  space_countsubs <- append(space_countsubs, list(counts[,grepl(i, meta$GroupedRegion)]))
}

# Re-leveling all factors so that no factor level has 0 samples
for (i in 1:length(space_metasubs)){
  for (j in colnames(space_metasubs[[i]])){
    space_metasubs[[i]][,j] <- factor(space_metasubs[[i]][,j])
  }
}

```

#### Verifying if all subsets are correct {style="background-color: #449944; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
for (i in 1:length(time_metasubs)){
  print(all(colnames(time_countsubs[[i]]) %in% rownames(time_metasubs[[i]]))) # same names
  print(all(colnames(time_countsubs[[i]]) == rownames(time_metasubs[[i]]))) # same order
}

for (i in 1:length(space_metasubs)){
  print(all(colnames(space_countsubs[[i]]) %in% rownames(space_metasubs[[i]]))) # same names
  print(all(colnames(space_countsubs[[i]]) == rownames(space_metasubs[[i]]))) # same order
}
```

# Exploratory analysis {style="background-color: #0049b7; color: #FFF685;  padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

#### Creating directories {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### Data distribution {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
sampDistri_func(meta, "GroupedAge","Region", "GroupedAge_Region.png")
sampDistri_func(meta, "Age","GroupedRegion", "Age_GroupedRegion.png")
sampDistri_func(meta, "GroupedAge","GroupedRegion", "GroupedAge_GroupedRegion.png")
sampDistri_func(meta, "GroupedAge","AuxCol", "GroupedAge_AuxCol.png")
sampDistri_func(meta, "AuxCol","GroupedRegion", "AuxCol_GroupedRegion.png")

sampDistri_func(meta, "GroupedAge","pH", "GroupedAge_pH.png")
sampDistri_func(meta, "GroupedAge","PMI", "GroupedAge_pH.png")
sampDistri_func(meta, "GroupedAge","RIN", "GroupedAge_pH.png")

sampDistri_func(meta, "GroupedRegion","pH", "GroupedRegion_pH.png")
sampDistri_func(meta, "GroupedRegion","PMI", "GroupedRegion_pH.png")
sampDistri_func(meta, "GroupedRegion","RIN", "GroupedRegion_pH.png")

# Remove the column "AuxCol" from the dataframe "meta" (no longer necessary)
# Arummar isso e colocando quando criando as listas de subsets: time_metasubs
#for (i in 1:length(time_metasubs)){
#  time_metasubs[[i]] <- time_metasubs[[i]] %>% select(-AuxCol)
#}
```

### DR - all samples - not adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

DR = Dimensionality reduction.

The idea is: the sample metadata will show all variables (such as region, age, pH, sex, ethnicity, etc.) that define or could define the gene expression profile of that sample. A good study design controls for all the variables that are not of interest for that study and only let the variables of interest control the expression profile. However, sometimes that is not done or its impossible to do, such as sex or ethnicity. Thus, by dimensionality reduction we can see if the variables (which are not of interest) have an impact in the expression profile of different samples in such way that they cluster together. This is a batch effect and it can be corrected statistically.
...
```{r}
adj_counts <- ComBat_seq(as.matrix(counts), batch = meta$pH, group=NULL)
...
### DR - all samples - adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
#### Creating batch adjusted time and space counts subsets {style="background-color: #449944; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px;"}
...
time_adjcts <- list()
for (i in levels(meta$GroupedAge)){
  subset <- adj_counts[,grepl(i, meta$GroupedAge)]
  time_adjcts <- append(time_adjcts, list(as.data.frame(subset)))
}

# No subset for ganglionic eminence because there is only one period with those samples; thus, no differential expression analysis can be performed.
space_adjcts <- list()
for (i in levels(meta$GroupedRegion)[!grepl("Ganglionic_eminence", levels(meta$GroupedRegion))]){
  subset <- adj_counts[,grepl(i, meta$GroupedRegion)]
  space_adjcts <- append(space_adjcts, list(as.data.frame(subset)))
}
```

#### Verifying if all batch adjusted subsets are correct {style="background-color: #449944; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px;"}

```{r}
for (i in 1:length(time_metasubs)){
  print(all(colnames(time_adjcts[[i]]) %in% rownames(time_metasubs[[i]]))) # same names
  print(all(colnames(time_adjcts[[i]]) == rownames(time_metasubs[[i]]))) # same order
}

for (i in 1:length(space_metasubs)){
  print(all(colnames(space_adjcts[[i]]) %in% rownames(space_metasubs[[i]]))) # same names
  print(all(colnames(space_adjcts[[i]]) == rownames(space_metasubs[[i]]))) # same order
}
```

### DR - Time - not adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px;"}
...
### DR - Time - adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### Cook's - Time- adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### MDP - Time- adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### DR - Space - not adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px;"}
...
### DR - Space - adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px;"}
...
### Cook's - Space- adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### MDP - Space - adjusted for batch effect {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
# Gene co-expression network analysis {style="background-color: #0049b7; color: #FFF685;  padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

#### Time - Normalized counts matrix - adjusted for batch effect {style="background-color: #33AA33; color: #FFF685; padding-top: 7px;   padding-right: 7px;   padding-bottom: 7px;   padding-left: 7px"}

Acredito que não seja uma boa ideia normalizar em 3 momentos diferentes (para a análise exploratória, para o deseq, e para o cemitool) com 2 ferramentas diferentes (seurat, deseq) a mesma matriz de contagens. PENSAR NUMA FORMA MELHOR!

```{r}
time_normbatchsubs <- list()  

for (i in 1:length(time_adjcts)){
  dds <- DESeqDataSetFromMatrix(countData = time_adjcts[[i]], colData = time_metasubs[[i]], design = ~1)
  dds <- DESeq(dds)
  ...
  time_normbatchsubs <- append(time_normbatchsubs, cts)
}
```

#### Space - Normalized counts matrix - adjusted for batch effect {style="background-color: #33AA33; color: #FFF685; padding-top: 7px;   padding-right: 7px;   padding-bottom: 7px;   padding-left: 7px"}

Acredito que não seja uma boa ideia normalizar em 3 momentos diferentes (para a análise exploratória, para o deseq, e para o cemitool) com 2 ferramentas diferentes (seurat, deseq) a mesma matriz de contagens. PENSAR NUMA FORMA MELHOR!

```{r}
space_normbatchsubs <- list()  

for (i in 1:length(space_adjcts)){
  dds <- DESeqDataSetFromMatrix(countData = space_adjcts[[i]], colData = space_metasubs[[i]], design = ~1)
  dds <- DESeq(dds)
  vsd <- vst(dds, blind=FALSE)
  mat <- as.data.frame(assay(vsd))
  ...
  space_normbatchsubs <- append(space_normbatchsubs, cts)
}
```

##### Proof of method {style="background-color: #FFFFFF; color: #000000; padding-top: 7px;   padding-right: 7px;   padding-bottom: 7px;   padding-left: 7px"}

```{r}

```

### Time subsets {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### Time - Modules {style="background-color: #33AA33; color: #FFF685; padding-top: 7px;   padding-right: 7px;   padding-bottom: 7px;   padding-left: 7px"}
...
for (i in 1:length(time_modules)){
  ...
  while (anyNA(genIntMod$pathway)){
    for (j in 1:length(genesofi$genesofinterest)){
      for (k in 1:length(genIntMod$genes)){
        if (is.na(genIntMod[k,"pathway"]) & grepl(genesofi[j,"genesofinterest"],genIntMod[k,"genes"])){
          genIntMod[k,"pathway"] <- as.character(genesofi[j,"pathway"])
        }
      }
    }
  }
...
### Space subsets {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### Space - Modules {style="background-color: #33AA33; color: #FFF685; padding-top: 7px;   padding-right: 7px;   padding-bottom: 7px;   padding-left: 7px"}
...
# Differential expression analysis {style="background-color: #0049b7; color: #FFF685;  padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
### Time - comparing all regions - Log2FoldChange and P-adj lists {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
for (i in 1:length(time_metasubs)){
  # Defining meta and counts dataframes
  #cts <- time_normbatchsubs[[i]] #com o limma-voom
  cts <- time_countsubs[[i]] #com o deseq2
  met <- time_metasubs[[i]]
  
  # Getting the name of the GroupedAge subset
  GA <- levels(met$GroupedAge)[1]
  
  # Defining log2FoldChange and p-adj empty dataframes
  log_df <- data.frame(matrix(nrow = nrow(cts), ncol = nlevels(met$GroupedRegion)))
  padj_df <- data.frame(matrix(nrow = nrow(cts), ncol = nlevels(met$GroupedRegion)))
  
  for(j in 1:nlevels(met$GroupedRegion)){
    # Grouped region its comparing
    reg <- levels(met$GroupedRegion)[j]
    ...
    # Running limma
    #
    #results <- as.data.frame(_______________)
    
    # Running DESeq2
    dds <- DESeqDataSetFromMatrix(countData = cts, colData = met, design = ~ pH + Group)
    dds <- DESeq(dds)
    res <- results(dds)
    
    # Filling up log_df and padj_df
    log_df[,j] <- res$log2FoldChange
    padj_df[,j] <- res$padj
  }
  
  # Correcting rows names
  rownames(log_df) <- rownames(res)
  rownames(padj_df) <- rownames(res)
  # Correcting columns names
  colnames(log_df) <- levels(met$GroupedRegion)
  colnames(padj_df) <- levels(met$GroupedRegion)
  
  # Adding dataframes to lists
  time_logsubs <- append(time_logsubs, list(log_df))
  time_padjsubs <- append(time_padjsubs, list(padj_df))
}
```

### Time - comparing only cortex - Log2FoldChange and P-adj lists {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
```{r}
time_cortexlogsubs <- list()
time_cortexpadjsubs <- list()

for (i in 1:length(time_countsubs)){
  # Defining meta and counts dataframes
  #cts <- time_normbatchsubs[[i]] #com o limma-voom
  cts <- time_countsubs[[i]] #com o deseq2
  met <- time_metasubs[[i]]
  ...
  # Just cortex samples
  cts <- cts[,grepl("lobe", met$GroupedRegion)]
  met <- met[grepl("lobe", met$GroupedRegion),]
  met$GroupedRegion <- factor(met$GroupedRegion)
  #for(k in 1:ncol(met)){met[,k] <- factor(met[,k])} # carefull not to take the order offf
  ...
  # Adding dataframes to lists
  time_cortexlogsubs <- append(time_cortexlogsubs, list(log_df))
  time_cortexpadjsubs <- append(time_cortexpadjsubs, list(padj_df))
}
```

### Space - comparing all ages - Log2FoldChange and P-adj lists {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

One heat map for each space subset, comparing all ages between each other.
...
for (i in 1:length(space_metasubs)){
  # Defining meta and counts dataframes
  #cts <- space_normbatchsubs[[i]] # for limma
  cts <- space_countsubs[[i]]
  met <- space_metasubs[[i]]
  
  # Getting the name of the GroupedAge subset
  GR <- levels(met$GroupedRegion)[1]
  
  # Defining log2FoldChange and p-adj empty dataframes
  log_df <- data.frame(matrix(nrow = nrow(cts), ncol = nlevels(met$GroupedAge)))
  padj_df <- data.frame(matrix(nrow = nrow(cts), ncol = nlevels(met$GroupedAge)))
  
  for(j in 1:nlevels(met$GroupedAge)){
    # Grouped age its comparing
    age <- levels(met$GroupedAge)[j]
    
    # Adding column "Group" to metadata
    met$Group <- ifelse(met$GroupedAge == age, age, "Control")
    met$Group <- factor(met$Group)
    met$Group <- relevel(met$Group, ref = "Control")
    
    # Running limma
    #
    #results <- as.data.frame(_______)
  ...
  rownames(padj_df) <- rownames(res)
  # Correcting columns names
  colnames(log_df) <- levels(met$GroupedAge)
  colnames(padj_df) <- levels(met$GroupedAge)
  
  # Adding dataframes to lists
  space_logsubs <- append(space_logsubs, list(log_df))
  space_padjsubs <- append(space_padjsubs, list(padj_df))
}
```

### Space - comparing only early stages - Log2FoldChange and P-adj lists {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}

One heat map for each space subset, comparing only early developmental ages between each other. So that the gene expression of older regions doesn't mask more subtle variability in early developmental stages.

```{r}

```

### Function to plot heatmaps {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
...
my_heatmaps <- function(type, list_log, list_padj, list_meta, wanted_genes, genes_category, sig, dir, hei, wid){
  ...
  for (i in 1:length(list_log)){
    ...
    if (wanted_genes == "mostDE"){
      log_df <- df[order(-rowSums(abs(log_df))), ]
      log_df <- head(log_df, 70)
    }else{
      # Select only wanted genes/rows
...
### Plotting Heatmaps {style="background-color: #F75990; color: #FFF685; padding-top: 15px;   padding-right: 15px;   padding-bottom: 15px;   padding-left: 15px"}
